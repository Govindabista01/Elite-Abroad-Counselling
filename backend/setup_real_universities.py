...
def main():
    print("Clearing all Universities from the database...")
    University.objects.all().delete()

    print("Adding verified universities to database...")
    added_count = 0

    for (country_name, name, website), slug, url in selected_unis:
        try:
            country = Country.objects.get(name__icontains=country_name)
        except Country.DoesNotExist:
            print(f"Skipping {name} - Country {country_name} not found.")
            continue
    ...
    print(f"\n✅ Finished! Added {added_count} universities.")
